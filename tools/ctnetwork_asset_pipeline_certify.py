...
def login():
    ...
    for attempt in range(1,121):
        try:
            S.cookies.clear()
            r=S.get(BASE+'/login',timeout=15); last=f'GET {r.status_code}'
            if r.status_code != 200: time.sleep(4); continue
            m=re.search(r'name="_xsrf" value="([^"]+)"',r.text)
            if not m: last='GET 200 without xsrf'; time.sleep(4); continue
            rr=S.post(BASE+'/login',data={'_xsrf':m.group(1),'password':PASSWORD,'next':'/'},timeout=20,allow_redirects=False); last=f'POST {rr.status_code}'
            if rr.status_code not in (200,302,303): time.sleep(4); continue
            xs=S.cookies.get('_xsrf'); h={'X-XSRFToken':xs} if xs else {}
            sr=S.get(BASE+'/api/status',headers=h,timeout=20); last=f'STATUS {sr.status_code}'
            if sr.status_code != 200: time.sleep(4); continue
            print(f'JUPYTER_AUTH_READY attempt={attempt}',flush=True); return h
        except Exception as e:
            last=repr(e); time.sleep(4)
...
ffmpeg -y -f lavfi -i 'testsrc2=size=640x360:rate=24' -t 6 -pix_fmt yuv420p -c:v libx264 -preset ultrafast -crf 24 "$CERT/visual.mp4" >/tmp/ctn-cert-video.log 2>&1
ffmpeg -y -f lavfi -i 'sine=frequency=440:sample_rate=48000:duration=6' -af 'volume=0.08' -c:a pcm_s16le "$CERT/narration.wav" >/tmp/ctn-cert-audio.log 2>&1
...
{"job_id":"$JOB","show":"CTNETWORK Certification","title":"Supplied Asset Production Pipeline Certification","description":"Internal readiness proof.","inputs":{"narration":"$CERT/narration.wav","visual":"$CERT/visual.mp4","thumbnail":"$CERT/thumbnail.jpg"},"shorts":{"clips":[{"start":0,"seconds":2,"title":"CERT A"},{"start":3,"seconds":2,"title":"CERT B"}]},"publish":false}
JSON
"$PY" "$ROOT/controller/ctnetwork_factory_v2.py" run-manifest "$CERT/manifest.json"
OUT="$ROOT/ready_for_approval/$JOB"
for f in master.mp4 short_01_9x16.mp4 short_02_9x16.mp4 thumbnail.jpg qc.json approval.json checksums.json shorts_manifest.json; do test -s "$OUT/$f"; done
python3 - <<'PY'
...
for name in ['master.mp4','short_01_9x16.mp4','short_02_9x16.mp4']:
 p=out/name; d=json.loads(subprocess.check_output(['ffprobe','-v','error','-show_streams','-show_format','-of','json',str(p)],text=True)); s=d.get('streams',[]); assert any(x.get('codec_type')=='video' for x in s),name; assert any(x.get('codec_type')=='audio' for x in s),name
 if name.startswith('short_'): hs.append(hashlib.sha256(p.read_bytes()).hexdigest())
assert len(set(hs))==2,hs
report={'mode':'supplied_assets','status':'PASS','job_id':'ctn-asset-pipeline-cert','master':str(out/'master.mp4'),'shorts':2,'shorts_unique':True,'qc_pass':True,'manual_approval_verified':True,'publish_locked':True,'ltx_generation_required':False,'narrator_generation_required':False}
